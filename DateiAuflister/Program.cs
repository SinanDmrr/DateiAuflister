...
class Program
{
    static void Main()
    {
        ...
        if (Directory.Exists(zielpfad))
        {
            ...
            string[] pdfDateien = Directory.GetFiles(zielpfad, "*.pdf")
                .OrderByDescending(f => File.GetCreationTime(f)) // Sortiere nach Erstellungsdatum
                .Select(f => $"{File.GetCreationTime(f):dd.MM.yyyy}_{Path.GetFileName(f)}")
                .ToArray();
